Namespace Util
    ...
    Public Class EncodingUtil
        ...
        ' 機能説明  : 文字コードを判定する
        ' 備考      :
        Public Shared Function JudgeCode(ByRef bytCode() As Byte) As String
            JudgeCode = "SJIS"
            Dim lngSJIS As Long
            Dim lngJIS As Long
            Dim lngEUC As Long
            ...
            lngJIS = JudgeJIS(bytCode, True) : System.Diagnostics.Trace.WriteLine("JIS :" & lngJIS)
            If lngJIS >= JUDGEFIX Then JudgeCode = "JIS" : Exit Function

            lngUNI = JudgeUNI(bytCode, True) : System.Diagnostics.Trace.WriteLine("UNI :" & lngUNI)
            If lngUNI >= JUDGEFIX Then JudgeCode = "UNICODE" : Exit Function

            lngUTF8 = JudgeUTF8(bytCode, True) : System.Diagnostics.Trace.WriteLine("UTF8:" & lngUTF8)
            If lngUTF8 >= JUDGEFIX Then JudgeCode = "UTF8" : Exit Function

            lngUTF7 = JudgeUTF7(bytCode, True) : System.Diagnostics.Trace.WriteLine("UTF7:" & lngUTF7)
            If lngUTF7 >= JUDGEFIX Then JudgeCode = "UTF7" : Exit Function

            lngSJIS = JudgeSJIS(bytCode, True) : System.Diagnostics.Trace.WriteLine("SJIS:" & lngSJIS)
            If lngSJIS >= JUDGEFIX Then JudgeCode = "SJIS" : Exit Function

            lngEUC = JudgeEUC(bytCode, True) : System.Diagnostics.Trace.WriteLine("EUC :" & lngEUC)
            If lngEUC >= JUDGEFIX Then JudgeCode = "EUC" : Exit Function
            System.Diagnostics.Trace.WriteLine("--------")

            If lngSJIS >= lngSJIS And lngSJIS >= lngUNI And lngSJIS >= lngJIS And
               lngSJIS >= lngUTF7 And lngSJIS >= lngUTF8 And lngSJIS >= lngEUC Then
                JudgeCode = "SJIS"
                Exit Function
            End If

            If lngUNI >= lngSJIS And lngUNI >= lngUNI And lngUNI >= lngJIS And
               lngUNI >= lngUTF7 And lngUNI >= lngUTF8 And lngUNI >= lngEUC Then
                JudgeCode = "UNICODE"
                Exit Function
            End If

            If lngJIS >= lngSJIS And lngJIS >= lngUNI And lngJIS >= lngJIS And
               lngJIS >= lngUTF7 And lngJIS >= lngUTF8 And lngJIS >= lngEUC Then
                JudgeCode = "JIS"
                Exit Function
            End If

            If lngUTF7 >= lngSJIS And lngUTF7 >= lngUNI And lngUTF7 >= lngJIS And
               lngUTF7 >= lngUTF7 And lngUTF7 >= lngUTF8 And lngUTF7 >= lngEUC Then
                JudgeCode = "UTF7"
                Exit Function
            End If

            If lngUTF8 >= lngSJIS And lngUTF8 >= lngUNI And lngUTF8 >= lngJIS And
               lngUTF8 >= lngUTF7 And lngUTF8 >= lngUTF8 And lngUTF8 >= lngEUC Then
                JudgeCode = "UTF8"
                Exit Function
            End If

            If lngEUC >= lngSJIS And lngEUC >= lngUNI And lngEUC >= lngJIS And
        ...
        ' 備考      :
        Private Shared Function JudgeSJIS(ByRef bytCode() As Byte,
        Optional ByVal fixFlag As Boolean = False) As Integer
            Dim i As Long
            Dim lngFit As Long
            Dim lngUB As Long
            ...
            For i = 0 To lngUB
                '81-9F,E0-EF(1バイト目)
                If (bytCode(i) >= &H81 And bytCode(i) <= &H9F) Or
                   (bytCode(i) >= &HE0 And bytCode(i) <= &HEF) Then
                    If i <= UBound(bytCode) - 1 Then
                        '40-7E,80-FC(2バイト目)
                        If (bytCode(i + 1) >= &H40 And bytCode(i + 1) <= &H7E) Or
                ...
                ElseIf (bytCode(i) >= &H20 And bytCode(i) <= &H7E) Then
                    ...
                    '00-1F, 7F(1バイト目)
                ElseIf (bytCode(i) >= &H0 And bytCode(i) <= &H1F) Or
                        bytCode(i) = &H7F Then
                    lngFit = lngFit + (1 * SingleByteWeight)
                End If
            Next i
        ...
        ' 備考      :
        Private Shared Function JudgeJIS(ByRef bytCode() As Byte,
        Optional ByVal fixFlag As Boolean = False) As Integer
            Dim i As Long
            Dim lngFit As Long
                ...
                Else
                    Select Case lngMode
                        Case JISMODE.ctrl, JISMODE.asci, JISMODE.roma
                            '00-1F,7F
                            If (bytCode(i) >= &H0 And bytCode(i) <= &H1F) Or
                                bytCode(i) = &H7F Then
                                lngFit = lngFit + (1 * SingleByteWeight)
                            End If
                            '20-7E
                        ...
                        Case JISMODE.kanO, JISMODE.kanN, JISMODE.kanH
                            If i <= UBound(bytCode) - 1 Then
                                '21-7E
                                If (bytCode(i) >= &H21 And bytCode(i) <= &H7E) And
        ...
        ' 備考      :
        Private Shared Function JudgeEUC(ByRef bytCode() As Byte,
        Optional ByVal fixFlag As Boolean = False) As Integer
            Dim i As Long
            Dim lngFit As Long
            Dim lngUB As Long
            ...
            For i = 0 To lngUB
                ...
                ElseIf bytCode(i) = &H8F Then
                    If i <= UBound(bytCode) - 2 Then
                        If (bytCode(i + 1) >= &HA1 And bytCode(i + 1) <= &HFE) And
        ...
        ' 備考      :
        Private Shared Function JudgeUNI(ByRef bytCode() As Byte,
        Optional ByVal fixFlag As Boolean = False) As Integer
            Dim i As Long
            Dim lngFit As Long
            Dim lngUB As Long
        ...
        ' 備考      :
        Private Shared Function JudgeUTF7(ByRef bytCode() As Byte,
        Optional ByVal fixFlag As Boolean = False) As Integer
            Dim i As Long
            Dim lngFit As Long
            Dim lngWrk As Long
            ...
            For i = 0 To lngUB
                ...
                ElseIf bytCode(i) = Asc("-") Then
                    ...
                    ElseIf lngWrk >= 4 And lngXB < 6 And
        ...
        ' 備考      :
        Private Shared Function JudgeUTF8(ByRef bytCode() As Byte,
        Optional ByVal fixFlag As Boolean = False) As Integer
            Dim i As Long
            Dim lngFit As Long
            Dim lngUB As Long
            ...
            For i = 0 To lngUB
                ...
                If (bytCode(i) And &HFC) = &HFC Then
                    If i <= UBound(bytCode) - 5 Then
                        If (bytCode(i + 1) >= &H80 And bytCode(i + 1) <= &HBF) And
                           (bytCode(i + 2) >= &H80 And bytCode(i + 2) <= &HBF) And
                           (bytCode(i + 3) >= &H80 And bytCode(i + 3) <= &HBF) And
                           (bytCode(i + 4) >= &H80 And bytCode(i + 4) <= &HBF) And
